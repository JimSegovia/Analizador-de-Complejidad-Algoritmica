...
public class AnalizadorComplejidad {
    ...
    public static void setTablapasos(VistaGeneral tabla) {
    vista = tabla;
}

     private static void log(String mensaje) {
        System.out.println(mensaje); // Seguir imprimiendo en consola
        
        // Si hay vista configurada, agregar a la tabla
        if (vista != null) {
            vista.agregarFilaPasos(mensaje);
        }
    }
    
    /* public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        log("Ingresa el codigo C++ (escribe 'END' en una nueva linea para finalizar):");
    ...
    public void procesarCodigo(StringBuilder codigo) {
        ...
        for (String ln : lineas) {
            ...
            if (ln.startsWith("#include") || 
                ...
                log("Ignorando línea de preprocesador/import: " + ln);
                continue;
            }
            ...
            if (esDeclaracionFuncion(ln) && !ln.contains("main")) {
                ...
                log(" Declarando funcion: " + nombreFuncion);
                continue;
            }
    ...
    if (tope.tipo == TipoBloque.IF && tope.cuerpoIfCerrado && !ln.startsWith("else")) {
        ...
        log(" IF sin ELSE terminado. Condición: " + tope.oeCondicion + 
        ...
        if (!pilaBloques.isEmpty() && 
            ...
            log(" IF dentro de ciclo. Sumando " + totalBloque + " al cuerpo del ciclo. Nuevo total del cuerpo: " + cicloActivo.oeCuerpo);
        } else if (enFuncion) {
            funcionesUsuario.get(funcionActual).oeTotal = funcionesUsuario.get(funcionActual).oeTotal.sumar(totalBloque);
            log(" Sumando " + totalBloque + " OEs a función " + funcionActual);
        } else {
    ...
    if (enFuncion && pilaBloques.isEmpty()) {
        log(" Cerrando función: " + funcionActual);
        enFuncion = false;
        funcionActual = null;
        continue;
    }
    ...
    if (!pilaBloques.isEmpty()) {
        ...
        if (bloqueActual.tipo == TipoBloque.FOR || bloqueActual.tipo == TipoBloque.WHILE) {
            ...
            if (!antesLlave.isEmpty()) {
                ...
                log(" Procesando codigo antes de }: " + antesLlave + " (" + totalOEAntesLlave + " OEs)");
            }

            // Cerrar ciclo
            bloqueActual = pilaBloques.pop();
            ResultadoComplejidad totalCiclo = calcularComplejidadCiclo(bloqueActual);
            log(" Cerrando ciclo " + bloqueActual.tipo + ". Complejidad total: " + totalCiclo);
            ...
            if (!pilaBloques.isEmpty() && 
                ...
                log(" Ciclo anidado detectado. El ciclo interior (" + totalCiclo + ") se suma como instruccion del ciclo padre");
            } else if (enFuncion) {
                funcionesUsuario.get(funcionActual).oeTotal = funcionesUsuario.get(funcionActual).oeTotal.sumar(totalCiclo);
                log(" Sumando " + totalCiclo + " OEs a función " + funcionActual);
            } else {
                totalOE = totalOE.sumar(totalCiclo);
                log(" Sumando al total: " + totalCiclo + ", Total actual: " + totalOE);
            }
            enCuerpo = !pilaBloques.isEmpty(); // Seguimos en cuerpo si hay más bloques
            ...
            if (!despuesLlave.isEmpty()) {
                log(" Procesando codigo después de }: " + despuesLlave);
        ...
        // Para IF: manejar lógica especial de IF-ELSE
            ...
            if (!antesLlave.isEmpty()) {
                ...
                bloqueActual.oeCuerpo = bloqueActual.oeCuerpo.sumar(totalOEAntesLlave);
                log(" Procesando codigo antes de } en IF: " + antesLlave + " (" + totalOEAntesLlave + " OEs)");
                log(" Total IF actualizado: " + bloqueActual.oeCuerpo);
            }
            
            bloqueActual.cuerpoIfCerrado = true;
            enCuerpo = false;
            log(" Cerrando cuerpo IF, esperando posible ELSE. IF tiene: " + bloqueActual.oeCuerpo + " OEs");
            ...
            if (despuesLlave.startsWith("else")) {
                ...
                enCuerpo = true;
                log(" Iniciando bloque ELSE en la misma línea");
                ...
                if (!contenidoDespuesElse.isEmpty()) {
                    ...
                    log(" Procesando contenido después de ELSE en la misma línea: " + contenidoDespuesElse + " (" + totalOEContenidoElse + " OEs)");
                    log(" Total ELSE actualizado: " + bloqueActual.oeCuerpoElse);
                }
            }
        }
    ...
    if (!antesLlave.isEmpty()) {
        ...
        log(" Procesando codigo antes de } en ELSE: " + antesLlave + " (" + totalOEAntesLlave + " OEs)");
        log(" Total ELSE actualizado: " + bloqueActual.oeCuerpoElse);
    }
    ...
    log(" Cerrando IF-ELSE completo. IF: " + bloqueActual.oeCuerpo + 
    ...
    if (!pilaBloques.isEmpty() && 
        ...
        log(" IF-ELSE dentro de ciclo. Sumando " + totalBloque + " al cuerpo del ciclo padre");
    } else if (enFuncion) {
        funcionesUsuario.get(funcionActual).oeTotal = funcionesUsuario.get(funcionActual).oeTotal.sumar(totalBloque);
        log(" Sumando " + totalBloque + " OEs a función " + funcionActual);
    } else {
        totalOE = totalOE.sumar(totalBloque);
        log(" Sumando al total: " + totalBloque + ", Total actual: " + totalOE);
    }
    ...
    continue;
            ...
            if (ln.startsWith("for") && ln.contains("(")) {
                ...
                pilaBloques.push(bloqueActual);
                enCuerpo = true;
                log(" Detectado ciclo FOR con rango: " + infoCiclo.rango);
                continue;
            }
            ...
            if (ln.startsWith("while") && ln.contains("(")) {
                ...
                pilaBloques.push(bloqueActual);
                enCuerpo = true;
                log(" Detectado ciclo WHILE con rango: " + infoCiclo.rango);
                continue;
            }
                ...
                if (!pilaBloques.isEmpty()) {
                    ...
                    if (bloqueActual.tipo == TipoBloque.IF && bloqueActual.cuerpoIfCerrado) {
                        ...
                        if (!contenidoDespuesElse.isEmpty()) {
                            ...
                            bloqueActual.oeCuerpoElse = bloqueActual.oeCuerpoElse.sumar(totalOEContenidoElse);
                            log(" Procesando contenido después de ELSE: " + contenidoDespuesElse + " (" + totalOEContenidoElse + " OEs)");
                            log(" Total ELSE actualizado: " + bloqueActual.oeCuerpoElse);
                        }
                    }
                }
            ...
            if (!oeLlamadaFuncion.esVacio()) {
                log(" Linea con llamada a funcion. OE normales: " + oeLinea + ", OE llamada: " + oeLlamadaFuncion);
            }
            ...
            if (enCuerpo) {
                if (enCuerpo) {
                    if (bloqueActual.enElse) {
                        ...
                        log(" Sumando " + totalOELinea + " OEs al ELSE. Total ELSE: " + bloqueActual.oeCuerpoElse);
                    } else {
                        bloqueActual.oeCuerpo = bloqueActual.oeCuerpo.sumar(totalOELinea);
                        log(" Sumando " + totalOELinea + " OEs al cuerpo. Total: " + bloqueActual.oeCuerpo);
                    }
                }
            } else if (enFuncion) {
                funcionesUsuario.get(funcionActual).oeTotal = funcionesUsuario.get(funcionActual).oeTotal.sumar(totalOELinea);
                log(" Sumando " + totalOELinea + " OEs a funcion " + funcionActual + ". Total funcion: " + funcionesUsuario.get(funcionActual).oeTotal);
            } else {
                totalOE = totalOE.sumar(totalOELinea);
                log(" Sumando " + totalOELinea + " OEs fuera de bloques. Total: " + totalOE);
            }
        }
        ...
        for (Map.Entry<String, Funcion> entry : funcionesUsuario.entrySet()) {
            log("Funcion " + entry.getKey() + ": " + entry.getValue().oeTotal);
        }
        log("============================\n");
        
        log("\n=== DEBUG FINAL ===");
log("Estado de la pila de bloques: " + pilaBloques.size());
...
log("===================\n");
    ...
    private static CicloInfo analizarFor(String ln) {
        ...
        log(" FOR - Init: " + info.inicializacion + ", Cond: " + info.condicion + ", Inc: " + info.incremento + ", Rango: " + info.rango);
    ...
    private static CicloInfo analizarWhile(String ln) {
        ...
        log(" WHILE - Cond: " + info.condicion + ", Rango: " + info.rango);
...
private static ResultadoComplejidad calcularComplejidadCiclo(Bloque bloque) {
    ...
    log(" Ciclo CORREGIDO - Init: " + info.inicializacion + 
    ...
    private static ResultadoComplejidad calcularLlamadaFuncion(String ln) {
        ...
        for (String nombreFuncion : funcionesUsuario.keySet()) {
            ...
            while (matcher.find()) {
                log(" Detectada llamada a funcion: " + nombreFuncion);
                
                // 1 OE por la llamada a funcion
                oeTotal = oeTotal.sumar(new ResultadoComplejidad(1));
                log(" +1 OE por la llamada a funcion");
                ...
                if (!parametros.isEmpty()) {
                    ...
                    log(" +" + oeParametros + " OEs por parametros");
                }
                ...
                log(" +" + oeCuerpoFuncion + " OEs del cuerpo de la funcion");
                
                log(" Total llamada a " + nombreFuncion + ": " + oeTotal + " OEs");
            }
        }
    ...
    private static ResultadoComplejidad calcularOE(String ln) {
        ...
        if (lnProcesada.trim().startsWith("return")) {
            ...
            if (!expresionReturn.isEmpty() && !expresionReturn.equals(";")) {
                ...
                log(" +" + oeExpresion + " OEs por expresion en return");
            }
            return oe;
        }
            ...
            // Operaciones aritméticas
                ...
                while (mArray.find()) {
                    ...
                    if (contenidoIndice.matches(".*[a-zA-Z].*")) {
                        oe = oe.sumar(new ResultadoComplejidad(1));
                        log(" +1 OE por acceso a array con variable: " + indiceCompleto);
                    } else {
                        log(" Ignorando acceso a array con índice numérico: " + indiceCompleto);
                    }
                }
...
static class ResultadoComplejidad {
    ...
    log(" Comparando max - A: " + a + " (orden " + ordenA + "), B: " + b + " (orden " + ordenB + ")");
    
    if (ordenA > ordenB) {
        log(" A tiene mayor orden de complejidad");
        return a;
    }
    if (ordenB > ordenA) {
        log(" B tiene mayor orden de complejidad");
        return b;
    }
